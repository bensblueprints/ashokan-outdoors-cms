...
import { ArrowLeft, ArrowRight, CheckCircle, Clock, Users, Fish } from 'lucide-react'
...
export default function PageContent({ section, settings }: { section: Section | null; settings: Record<string, string> }) {
  const title = section?.title || 'Catskills Fly Fishing Instruction'
  const subtitle = section?.subtitle || 'Learn to fly fish in the birthplace of American fly fishing with licensed Catskill Mountain guides.'
  const heroImage = section?.image || '/images/hiking.jpg'
  const bodyParagraphs = section?.body?.split('\n\n').filter(Boolean) || [
    'Whether you are looking to refine your fly fishing skills or if you are just starting out — our knowledgeable fly fishing instructors will help you enhance your proficiency.',
  ]
              ...
              <div className="glass rounded-2xl p-8 border-amber-500/20">
                ...
                <div className="space-y-5">
                  <div className="border-l-2 border-amber-500/40 pl-4">
                    ...
                    <p className="font-body text-stone-400 text-sm leading-relaxed mb-2">Mandatory for anglers age 16 and over. Single and multi-day options available for NY residents and non-residents.</p>
                  ...
                  <div className="border-l-2 border-amber-500/40 pl-4">
                    ...
                    <p className="font-body text-stone-400 text-sm leading-relaxed mb-2">Ashokan Outdoors is licensed by NYC DEP to guide clients on NYC watershed properties, but permits are required to enter these lands. The NYC Watershed Permit is free.</p>
            ...
            <div className="space-y-6">
              <div className="glass rounded-2xl p-8 sticky top-24">
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex items-center gap-3 text-stone-300"><Clock className="w-5 h-5 text-forest-400" /><span className="font-body">90 minutes</span></div>
                  <div className="flex items-center gap-3 text-stone-300"><Users className="w-5 h-5 text-forest-400" /><span className="font-body">Individual or small group</span></div>
                  <div className="flex items-center gap-3 text-stone-300"><Fish className="w-5 h-5 text-forest-400" /><span className="font-body">All equipment included</span></div>
                </div>
                ...
                <p className="font-body text-xs text-stone-500 mt-4 text-center">20% deposit required upon booking. Balance due day of trip.</p>
              </div>
